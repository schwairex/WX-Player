...
if(pending is not null)
{
    ...
    await UpdateActivation.ActivateAsync(data,update,new Version(1,7,0));
}
...
await File.WriteAllTextAsync(Path.Combine(data,"fixture-activated.json"),JsonSerializer.Serialize(new{previousExited,version="1.7.0",runs=runs+1,pointer=File.Exists(UpdateActivation.Pointer(data)),dataPreserved=File.Exists(Path.Combine(data,"library.db"))}));
return 0;
