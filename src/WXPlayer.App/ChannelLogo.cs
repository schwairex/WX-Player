...
public sealed class ChannelLogo : Grid
{
    ...
    private async void Refresh()
    {
        ...
        var bitmap=await Cache.GetOrAdd(url,_=>Download(uri));
    ...
    private static async Task<BitmapSource?> Download(Uri uri)
    {
        ...
        try
        {
            ...
            return await Task.Run(()=>{data.Position=0;var bitmap=new BitmapImage();bitmap.BeginInit();bitmap.CacheOption=BitmapCacheOption.OnLoad;bitmap.DecodePixelWidth=96;bitmap.StreamSource=data;bitmap.EndInit();bitmap.Freeze();return (BitmapSource)bitmap;});
