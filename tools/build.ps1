...
if (-not $SkipTests) {
    & dotnet run --project (Join-Path $repo 'tests\WXPlayer.Tests') -c Release -- (Join-Path $outDir 'test-results.json')
...
& dotnet publish (Join-Path $repo 'src\WXPlayer.App\WXPlayer.App.csproj') -c Release -r win-x64 --self-contained true -p:VlcWindowsX86Enabled=false -p:VlcWindowsArm64Enabled=false -p:DebugType=None -o $portable
...
[IO.File]::WriteAllText($hashSource,('internal static class PayloadInfo { public const string Sha256 = "' + $hash + '"; }'))
